...
export const Achievements = () => {
  ...
  return <main className="platform-page"><aside className="platform-sidebar" aria-label="РќР°РІРёРіР°С†РёСЏ РїР»Р°С‚С„РѕСЂРјС‹"><section className="platform-sidebar-block platform-sidebar-top"><div className="platform-logo-frame"><img src={logo} alt="KTSThub" /></div><nav className="platform-menu">{topMenuItems.map(renderMenuButton)}</nav></section><section className="platform-sidebar-block platform-sidebar-bottom"><nav className="platform-menu">{bottomMenuItems.map(renderMenuButton)}</nav></section></aside><section className="platform-workspace"><header className="platform-header"><div className="platform-content-pill"><span>{activeTopItem.label}</span></div><div className="platform-search-group"><label className="platform-search" aria-label="РџРѕРёСЃРє"><input type="search" placeholder="РџРѕРёСЃРє" /><span className="platform-round-button"><Icon name="search" /></span></label><button type="button" className="platform-round-button" aria-label="РР·Р±СЂР°РЅРЅРѕРµ"><Icon name="favorite" /></button><button type="button" className="platform-round-button" aria-label="РЈРІРµРґРѕРјР»РµРЅРёСЏ"><Icon name="bell" /></button></div><div className="platform-user-card"><div className="platform-user-text"><strong>{user.lastName} {user.firstName}</strong><span>{user.email}</span></div><img className="platform-avatar" src={avatarSrc} alt={`${user.lastName} ${user.firstName}`} onError={() => setIsAvatarBroken(true)} /></div></header><section className="page-placeholder-card"><h1>РђС‡РёРІРєРё</h1><p>Р—РґРµСЃСЊ Р±СѓРґСѓС‚ РѕС‚РѕР±СЂР°Р¶Р°С‚СЊСЃСЏ РЅР°РіСЂР°РґС‹, Р±РµР№РґР¶Рё, СѓСЂРѕРІРЅРё РґРѕСЃС‚РёР¶РµРЅРёР№ Рё РёСЃС‚РѕСЂРёСЏ РїСЂРѕРіСЂРµСЃСЃР° РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ РЅР° С…Р°РєР°С‚РѕРЅР°С….</p></section></section></main>;
};
